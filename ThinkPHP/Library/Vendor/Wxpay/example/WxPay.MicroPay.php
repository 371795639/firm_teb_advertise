...
class MicroPay
{
	 ...
	 * @return 返回查询接口的结果
	 */
	public function pay($microPayInput,$vip_call_num)
	{
		//①、提交被扫支付
	//	$data=$microPayInput->GetVip_call_no();
	//	print_r($data);
		...
		//③、确认支付是否成功
		//$queryTimes = 10;
		for($queryTimes = 0;$queryTimes<10;$queryTimes++)
		{
			$succResult = 0;
			$queryResult = $this->query($out_trade_no, $succResult);
			//echo "<script>console.log(1)</script>";
			//var_dump($succResult);die;
			//如果需要等待1s后继续
			if($succResult == 2){
				sleep(2);
				continue;
				...
				{
					/**********进行会员手机号码和订单号以及金额的存储*********
					 *$result["transaction_id"] //交易订单号
                     *$result["total_fee"]      //交易金额
					 *$result["time_end"]       //交易时间
					 *$vip_call_num             //会员手机号
					 /*进行会员手机号码和订单号以及金额的存储****************/
				}
	...
	public function query($out_trade_no, &$succCode)
	{
			...
			&& $result["result_code"] == "SUCCESS")
		{
			...
			//用户支付中
				...
				$succCode = 2;
				return false;
			}else if($result['trade_state']=="NOTPAY" || $result['trade_state']=="PAYERROR"){
				$succCode = 0;
				return false;
			}
		}
