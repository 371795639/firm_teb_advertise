...
class TaskController extends HomeController {
    ...
    public function index(){
        ...
        p('递归查找其下推广专员。');
        $dbStaff    = D('Staff');
        $tgOne      = $dbStaff  -> get_staff_by_referee($_SESSION['userid'],'select','1');
        $tgTwo      = $dbStaff  -> get_staff_by_referee($tgOne,'select','1');
        $tgThr      = $dbStaff  -> get_staff_by_referee($tgTwo,'select','1');
        $tgAll      = array_merge($tgOne,$tgTwo,$tgThr);
        p($tgOne);p($tgTwo);p($tgThr);p($tgAll);
        */
        $this           -> taskWhats();                                                     //检测任务的完成情况
